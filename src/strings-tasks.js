...
function findLongestWord(sentence) {
  let max = 0;
  sentence.split(' ').forEach((a, index) => {
    if (a.length > sentence.split(' ')[max].length) {
      max = index;
    }
  });
  return sentence.split(' ')[max];
}
...
function encodeToRot13(str) {
  ...
  let result = '';
  strChars.forEach((element) => {
    if (
      element.toUpperCase().charCodeAt(0) >= 65 &&
      element.toUpperCase().charCodeAt(0) <= 90
    ) {
      if (element.toUpperCase().charCodeAt(0) > 77) {
        if (element === element.toUpperCase()) {
          result += String.fromCharCode(
            element.toUpperCase().charCodeAt(0) - 13
          );
        } else {
          result += String.fromCharCode(
            element.toUpperCase().charCodeAt(0) - 13
      ...
      } else if (element.toUpperCase().charCodeAt(0) < 78) {
        if (element === element.toUpperCase()) {
          result += String.fromCharCode(
            element.toUpperCase().charCodeAt(0) + 13
          );
